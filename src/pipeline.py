...
def load_cached_metadata(imdb_id: str, outputs_dir: Path = OUTPUTS_DIR) -> dict | None:
    if not imdb_id:
        return None
    padded_id = imdb_id.zfill(7) if imdb_id.isdigit() else imdb_id
    matches = list(outputs_dir.glob(f"*{padded_id}*.json.gz")) + list(
        outputs_dir.glob(f"*{padded_id}*.json")
    )
    if not matches:
        matches = list(outputs_dir.glob(f"{imdb_id}.json.gz")) + list(
            outputs_dir.glob(f"{imdb_id}.json")
        )
    if matches:

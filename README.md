...
- **Native Gemini LLM Integration**: Migrated the synopsis generation engine to the Google Gemini `generateContent` API, featuring a reliable fallback sequence (`flash-lite` -> `latest` -> `pro`), enriching outputs with loglines, themes, compliance flags, and character notes.
- **Fully Automated Pipeline**: Removed manual LLM and Transformer UI toggles, enforcing a fully automated NLP/LLM execution pipeline for every script analysis.
- **Robust Character Name Parsing**: Implemented robust regex-based speaker normalization to effectively strip OCR artifacts, trailing punctuation, and audio modifiers (e.g., `(V.O.)`, `(CONT'D)`) for cleaner character profiles.
